...
```
If you want the backend to create or refresh that admin automatically on startup, put those values in `.env` and restart the backend.
...
You can also keep using the script, now powered by the same environment variables:
```bash
docker compose exec backend python bootstrap_admin.py
```
