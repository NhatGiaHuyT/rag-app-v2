...
from app.services.access_control import (
    ...
    filter_accessible_knowledge_bases,
)
...
def serialize_document(document: Document) -> dict:
    return {
        ...
        "permissions": [serialize_permission(permission) for permission in document.permissions],
    }


def serialize_knowledge_base(kb: KnowledgeBase, visible_documents: List[Document] | None = None) -> dict:
    documents = visible_documents if visible_documents is not None else kb.documents
    return {
        "id": kb.id,
        "name": kb.name,
        ...
        "permissions": [serialize_permission(permission) for permission in kb.permissions],
    }
...
def get_knowledge_bases(
    ...
    accessible = filter_accessible_knowledge_bases(db, current_user, knowledge_bases)
    return [
        serialize_knowledge_base(kb, filter_accessible_documents(db, current_user, kb.documents))
...
def get_knowledge_base(
    *,
    ...
    if not kb or not can_access_knowledge_base(db, current_user, kb):
        log_access(db, current_user, "knowledge_base", "read", resource_id=str(kb_id), success=False, failure_reason="forbidden_or_missing")
        db.commit()
        raise HTTPException(status_code=404, detail="Knowledge base not found")

    visible_documents = filter_accessible_documents(db, current_user, kb.documents)
    log_access(db, current_user, "knowledge_base", "read", resource_id=str(kb_id), success=True)
    db.commit()
    return serialize_knowledge_base(kb, visible_documents)
...
def update_knowledge_base(
    *,
    ...
    return serialize_knowledge_base(kb, filter_accessible_documents(db, current_user, kb.documents))
...
async def get_document(
    *,
    ...
    if not document or not can_access_document(db, current_user, document):
        log_access(db, current_user, "document", "read", resource_id=str(doc_id), success=False, failure_reason="forbidden_or_missing")
        db.commit()
        raise HTTPException(status_code=404, detail="Document not found")

    log_access(db, current_user, "document", "read", resource_id=str(doc_id), success=True)
    db.commit()
...
@router.put("/{kb_id}/permissions", response_model=KnowledgeBaseResponse)
def update_knowledge_base_permissions(
    *,
    ...
    db.query(KnowledgeBasePermission).filter(KnowledgeBasePermission.knowledge_base_id == kb_id).delete()

    for user_id in payload.user_permissions:
        if user_id == kb.user_id:
            continue
        db.add(KnowledgeBasePermission(
            knowledge_base_id=kb_id,
            user_id=user_id,
            permission_level="viewer",
        ))

    for user_id in payload.editor_user_ids:
        if user_id == kb.user_id:
            continue
        permission = (
...
@router.put("/{kb_id}/documents/{doc_id}/permissions", response_model=DocumentResponse)
def update_document_permissions(
    *,
    ...
    if not document or not can_edit_document(db, current_user, document):
        raise HTTPException(status_code=404, detail="Document not found")
    ...
    for user_id in payload.user_permissions:
        if user_id == document.knowledge_base.user_id:
            continue
    ...
    for user_id in payload.editor_user_ids:
        ...
        permission = (
            ...
            .filter(DocumentPermission.document_id == doc_id, DocumentPermission.user_id == user_id)
            .first()
        )
        if permission:
            permission.permission_level = "editor"
